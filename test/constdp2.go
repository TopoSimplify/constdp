...
test("constrained dp simplification", func (t) {
  ...
  async.waterfall([
      func (cb) {//stream
        ...
        var constlist = []
        var filepath = path.join(__dirname, "consts2.txt")
  ...
  func client_simplify(tree, rtree) {
    ...
    if rtree.len(itemlist) == 0 {
      console.log('tails...')
      simplx.avoid_self = true
      simplx.preserve_complex = false
    }
    else {
      console.log('heads...')
      simplx.avoid_self = true
      simplx.preserve_complex = true
    }
    ...
    if len(genpoly) {
      ...
      t.ok(!poly_geom.is_simple(), 'not simple')
      //self intersects
      if rtree.len(itemlist) == 0 {
        t.ok(!genpoly_geom.is_simple(), 'not simple')
        t.Assert(genpoly_geom.self_intersection().length, 1)
      }
      else {
        t.ok(!genpoly_geom.is_simple(), 'not simple')
        t.Assert(genpoly_geom.self_intersection().length, 1)
