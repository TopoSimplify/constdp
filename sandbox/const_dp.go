...
func approx(seg []*Point) []*Point {
    coords := make([]*Point , len(seg))
    for i, pt := range seg {
        coords[i] = pt.Clone()
    }
    eps := 1e-8
    v := vect.NewVect(&vect.Options{A:coords[0], B:coords[1]})
    M := v.M()
    M = M - eps
    D := v.D()

    v2 := vect.NewVect(&vect.Options{A:coords[0], M:&M, D: &D})
    at := v2.B()
    coords[1] = &at
    return coords
}
...
func main() {
    ...
    var coord = []*Point{{0, 0}, {2, 2}, {3, 5}, {7, 8}}
    ln2 := NewLineString(coord)
    seg := approx(coord[1:3])
    fmt.Println(seg[1], "=> " , coord[2], seg[1].Equals(coord[2]))
    fmt.Println(seg)
    fmt.Println(ln2)
